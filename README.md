...
- Creating a calendar returns two secrets exactly once: a management link and a default subscription URL.
...
- Event CRUD with optional location, description, and recurrence rule; calendar rename and timezone change.
...
public/                   Landing page, manager page, styles, i18n
...
The smoke test covers calendar creation, session exchange, CSRF rejection, event create/edit/delete, ICS output (UID stability, RRULE, all-day `VALUE=DATE`, escaping, headers), subscription regenerate/revoke, and management-token rotation.
...
`slug` is optional; an empty or omitted value is derived from the name. `timezone` must be an IANA zone name. The response returns `manageUrl` and `subscriptionUrl` once; neither token can be read back later.
